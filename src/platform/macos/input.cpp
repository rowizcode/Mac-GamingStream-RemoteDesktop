...
const KeyCodeMap kKeyCodesMap[] = {
  ...
  int alloc_gamepad(input_t &input, const gamepad_id_t &id, const gamepad_arrival_t &metadata, feedback_queue_t feedback_queue) {
    ...
    // The generic input layer owns globalIndex and expects alloc_gamepad() to
    // return 0 on success. Returning the selected slot here caused every slot
    // except slot 0 to be treated as a failure, leaving orphaned HID devices
    // behind when Moonlight sessions overlapped.
    const int slot = id.globalIndex;
    if (slot < 0 || slot >= MAX_GAMEPADS || macos_input->hid_gamepads[slot]) {
      BOOST_LOG(warning) << "Unable to allocate macOS virtual gamepad slot "sv << slot;
      return -1;
    }

    HIDGamepad *gamepad = [[HIDGamepad alloc] initWithIndex:slot];
    if (![gamepad createDevice]) {
      [gamepad release];
      BOOST_LOG(warning) << "Unable to allocate macOS virtual gamepad slot "sv << slot;
      return -1;
    }

    macos_input->hid_gamepads[slot] = gamepad;
    BOOST_LOG(info) << "Gamepad "sv << slot << " allocated (IOHIDUserDevice mode)"sv;
    return 0;
  }

  void free_gamepad(input_t &input, int nr) {
    auto macos_input = static_cast<macos_input_t *>(input.get());
    if (nr < 0 || nr >= MAX_GAMEPADS || !macos_input->hid_gamepads[nr]) {
      return;
    }
  ...
  input_t input() {
    ...
    // Cursor warping does not require Accessibility, while injected mouse-down
    // and mouse-up events do.  Log the real trust state so a movable cursor can
    // no longer be mistaken for working click permission.
    const void *prompt_keys[] = {kAXTrustedCheckOptionPrompt};
    const void *prompt_values[] = {kCFBooleanTrue};
    const auto prompt_options = CFDictionaryCreate(
      kCFAllocatorDefault,
      prompt_keys,
      prompt_values,
      1,
    ...
    const bool accessibility_trusted = AXIsProcessTrustedWithOptions(prompt_options);
    CFRelease(prompt_options);
    BOOST_LOG(info) << (accessibility_trusted ?
      "macOS Accessibility permission is active"sv :
      "macOS Accessibility permission is NOT active; remote clicks/keys will be blocked"sv);

    macos_input->hid_available = [HIDGamepad isAvailable];
    BOOST_LOG(info) << (macos_input->hid_available ?
      "IOHIDUserDevice virtual gamepad support is available"sv :
      "IOHIDUserDevice virtual gamepad support is unavailable"sv);
  ...
  std::vector<supported_gamepad_t> &supported_gamepads(input_t *input) {
    static std::vector gamepads {
      supported_gamepad_t {"auto", [HIDGamepad isAvailable], "gamepads.macos_hid"}
    };
